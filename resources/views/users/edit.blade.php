            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                        placeholder="{{ __('Nama Lengkap') }}" value="{{ old('name', $user->name) }}" required>
                </div>
                @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                        placeholder="{{ __('Alamat Email') }}" value="{{ old('email', $user->email) }}" required>
                </div>
                @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="phone_number" name="phone_number"
                        ...
                        placeholder="{{ __('Nomor Telepon') }}" value="{{ old('phone_number', $user->phone_number) }}"
                        required>
                </div>
                @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
