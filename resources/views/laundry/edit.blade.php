...
@section('content')
    ...
    <div class="page-body">
        ...
        <form action="{{ route('laundry.update', $laundry) }}" method="POST" class="card" autocomplete="off"
            ...
            <div class="card-body">
                <div class="mb-3">
                    <label class="form-label required">{{ __('Nama Laundry') }}</label>
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                        placeholder="{{ __('Nama Laundry') }}" value="{{ old('name', $laundry->name) }}" required>
                </div>
                @error('name')
                ...
                <div class="mb-3">
                    <label class="form-label required">{{ __('Pemilik') }}</label>
                ...
                <div class="mb-3">
                    <label class="form-label required">{{ __('Nomor Izin') }}</label>
                    <input type="text" name="no_izin" class="form-control @error('no_izin') is-invalid @enderror"
                        placeholder="{{ __('Nomor Izin') }}" value="{{ old('no_izin', $laundry->no_izin) }}" required>
                </div>
                @error('no_izin')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="mb-3">
                    <label class="form-label required">{{ __('Alamat') }}</label>
                    <input type="text" name="address" class="form-control @error('address') is-invalid @enderror"
                        placeholder="{{ __('Alamat') }}" value="{{ old('address', $laundry->address) }}" required>
                </div>
                @error('address')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="mb-3">
                    <label class="form-label required">{{ __('Kecamatan') }}</label>
                    <input type="text" name="district" class="form-control @error('district') is-invalid @enderror"
                        placeholder="{{ __('Kecamatan') }}" value="{{ old('district', $laundry->district) }}" required>
                </div>
                @error('district')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="mb-3">
                    <label class="form-label required">{{ __('Kabupaten / Kota') }}</label>
                    <input type="text" name="city" class="form-control @error('city') is-invalid @enderror"
                        placeholder="{{ __('Kabupaten / Kota') }}" value="{{ old('city', $laundry->city) }}" required>
                </div>
                @error('city')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="d-flex" style="gap: 0.75rem">
                    ...
                    <div class="mb-3 w-100">
                        ...
                        <input type="text" name="long" class="form-control @error('long') is-invalid @enderror"
                            placeholder="{{ __('Longitude') }}" id="long" value="{{ old('long', $laundry->long) }}"
                            required>
                    </div>
                    @error('long')
                        <div class="invalid-feedback">{{ $message }}</div>
...
@section('custom_scripts')
    ...
    <script>
        ...
        async function showPosition(position) {
            let coords = {
                lat: @json(old('lat', $laundry->lat)) ?? position.coords.latitude,
                long: @json(old('long', $laundry->long)) ?? position.coords.longitude
            }
